...
{foreach $push_blocks as $index => $entry}
	...
	{switch match=$account.Type}
		...
		{case match='facebook_feed'}
			{* images dans relation d'objets *}
			{def $fb_pic = 0 $image_node = 0}
			{if and(
				ezini_hasvariable($entry, 'attrId_picture', 'ngpush.ini'), 
				ezini($entry, 'attrId_picture', 'ngpush.ini')[$node.class_identifier], 
				$node.data_map[ezini($entry, 'attrId_picture', 'ngpush.ini')[$node.class_identifier]].content.relation_list|count
			)}
				{set 
					$image_node = fetch('content', 'node', hash('node_id', $node.data_map[ezini($entry, 'attrId_picture', 'ngpush.ini')[$node.class_identifier]].content.relation_list.0.node_id))
					$fb_pic = concat(
							...
							<div class="account-body float-break"{if $active_blocks|contains($entry)|not} style="display:none;"{/if}>
								...
								<form action="">
									...
									<p>
										...
										<img src="{concat("/", $image_node.data_map.image.content['ngpushthumb'].url)}" alt="" />
									</p>
									{/if}
									<p>
